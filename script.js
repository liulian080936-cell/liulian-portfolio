...
function initHomeProjectHoverSlides() {
  const hoverQuery = window.matchMedia("(hover: hover) and (pointer: fine)");
  const reducedMotionQuery = window.matchMedia("(prefers-reduced-motion: reduce)");
  const manifest = Array.isArray(window.__PROJECT_MANIFEST__)
    ? window.__PROJECT_MANIFEST__
    : [];
  const galleries = new Map(manifest.map((project) => [project.slug, project.images || []]));
  const activeSlideshows = new Set();
  const cards = document.querySelectorAll(
    "body[data-page='home'] .cover-cloud-card[href*='project.html?slug=']",
  );

  const canPlay = () => hoverQuery.matches && !reducedMotionQuery.matches;

  cards.forEach((card) => {
    const media = card.querySelector(".cover-cloud-card-media");
    if (!media || media.dataset.hoverSlideshow === "ready") return;

    const url = new URL(card.getAttribute("href"), window.location.href);
    const gallery = galleries.get(url.searchParams.get("slug")) || [];
    const frames = gallery.filter(Boolean);
    if (!frames.length) return;

    media.dataset.hoverSlideshow = "ready";
    media.dataset.hoverSlideCount = String(frames.length);

    const slideLayers = [0, 1].map(() => {
      const image = document.createElement("img");
      image.className = "cover-cloud-card-hover-slide";
      image.alt = "";
    ...
    const preloadFrame = (index) => {
      ...
      if (!source || preloadedFrames.has(source)) return;

      preloadedFrames.add(source);
      const image = new Image();
      image.decoding = "async";
      image.src = source;
    };

    const showFrame = (source, token) => {
      const nextLayer = activeLayer === 0 ? 1 : 0;
      const nextImage = slideLayers[nextLayer];
      const previousImage = activeLayer >= 0 ? slideLayers[activeLayer] : null;

      const reveal = () => {
        if (token !== playToken || !card.classList.contains("is-hover-slideshow-active")) {
          return;
        }

        nextImage.style.zIndex = "2";
        previousImage?.style.setProperty("z-index", "1");
        previousImage?.classList.add("is-exiting-up");
        nextImage.classList.remove("is-visible", "is-exiting-up");
        ...
        if (previousImage) {
          window.setTimeout(() => {
            ...
            previousImage.classList.remove("is-visible", "is-exiting-up");
          }, 460);
        }
    ...
    const scheduleNextFrame = (token, delay = 520) => {
      ...
      timerId = window.setTimeout(() => {
        ...
        showFrame(frames[frameIndex], token);
        frameIndex = (frameIndex + 1) % frames.length;
        preloadFrame(frameIndex);
        scheduleNextFrame(token, 1380);
      }, delay);
    };

    const start = () => {
      if (!canPlay() || card.classList.contains("is-hover-slideshow-active")) return;
      ...
      preloadFrame(0);
      preloadFrame(1);
      card.classList.add("is-hover-slideshow-active");
      activeSlideshows.add(card);
      scheduleNextFrame(playToken);
    };

    const stop = () => {
      if (!card.classList.contains("is-hover-slideshow-active")) return;

      playToken += 1;
      window.clearTimeout(timerId);
      timerId = 0;
      activeLayer = -1;
      card.classList.remove("is-hover-slideshow-active");
      slideLayers.forEach((image) => {
        image.classList.remove("is-visible", "is-exiting-up");
        image.style.removeProperty("z-index");
      });
      activeSlideshows.delete(card);
    };
  ...
  const stopAllSlideshows = () => {
    activeSlideshows.forEach((card) => {
      card.dispatchEvent(new PointerEvent("pointerleave"));
    });
  };

  hoverQuery.addEventListener("change", stopAllSlideshows);
  reducedMotionQuery.addEventListener("change", stopAllSlideshows);
  document.addEventListener("visibilitychange", () => {
    if (document.hidden) stopAllSlideshows();
  });
}
